...
// Example implementation showing how body region data refines clinical questioning
...
function getRegionSpecificQuestions(
  ...
  // LIMB PAIN + LEFT/RIGHT ARM = DVT vs Musculoskeletal
  if (
    complaint === 'LIMB_PAIN' && 
...
export function calculateTriage(state: IntakeState): {
  ...
  if (complaint === 'CHEST_PAIN' && bodyRegion === 'CHEST') {
    const severity = answers['severity'] as number;
    if (severity >= 7) {
      return {
        level: 'IMMEDIATE',
  ...
  if (
    ...
    (answers['numbness'] === true || answers['bowel_bladder'] === true)
  ) {
    return {
      level: 'URGENT',
...
export function generateSummary(state: IntakeState): string {
  ...
  // Key details
  if (answers['onset']) {
    parts.push(`Onset: ${answers['onset']}`);
  }
  ...
  if (answers['nausea']) symptoms.push('nausea');
  if (answers['sweating']) symptoms.push('sweating');
  if (answers['shortness_of_breath']) symptoms.push('dyspnea');
...
/**
 * Example usage in your intake flow
 */
export function exampleUsage() {
  // 1. User selects complaint
  let state: IntakeState = {
    phase: 'COMPLAINT_SELECT',
    complaint: 'ABDOMINAL_PAIN',
    answers: {},
    redFlags: [],
    language: 'en',
  ...
  // 2. System shows body map (if required)
  state.phase = 'BODY_MAP';
  
  // 3. User selects region
  state.bodyRegion = 'LOWER_ABDOMEN';
  state.bodySide = 'RIGHT';
  state.phase = 'COMPLAINT_TREE';
  
  // 4. Get questions based on complaint + region
  const questions = getQuestions(state);
  // Returns: base questions + appendicitis screening
  ...
  // 6. Calculate triage
  const triage = calculateTriage(state);
  // Result: URGENT - possible appendicitis
  
  // 7. Generate summary for doctor
